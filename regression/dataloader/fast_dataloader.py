...
def gain_x_y_data():
    ...
    root = 'D:/Python项目/regression/data/predict_data/fast/'
...
def gain_x_y_true_weight_data():
    ...
    root = 'D:/Python项目/regression/data/test_data/fast/'
    for filepath, dirnames, filenames in os.walk(root):
        for filename in filenames:
            f = open(root + filename, encoding='utf-8')
            x_temp = []
...
def gain_coiled_crest_segment(x, y):    # 这里就是整段的波峰及其时间，该函数是获取分割线及其大于分割线的数据
    ...
    # 获取最长波峰段
    temp_first = 0      # 记录开始时间
    temp_last = len(y)       # 记录结束时间
    continue_time = 0   # 记录持续时间
    book = 0
    for i in range(len(y)):
        ...
        if y[i] >= mean_extremum_value and book == 0 and i != len(y) - 1:
            ...
            temp_last = i
            book = 0
            if temp_last - temp_first > continue_time:
                coiled_crest_segment[0] = temp_first
                coiled_crest_segment[1] = temp_last
                continue_time = temp_last - temp_first

    # 记录最长波峰段的最大最小值
    # crest_segment_max_value = np.max(y[temp_first: temp_last+1])
    # crest_segment_min_value = np.min(y[temp_first: temp_last+1])

    crest_segment_after_y = y[temp_first: temp_last+1]
    crest_segment_after_x = x[temp_first: temp_last+1]
    ...
    for i in range(len(crest_segment_after_y)):
        if crest_segment_after_y[i] > max_value * 0.95:
            crest_segment_y.append(crest_segment_after_y[i] * 0.95)
    ...
    print(crest_segment_y)

    mean_crest = np.mean(crest_segment_y)  # 获取大于分界线值的平均值
...
def gain_crest_segment(x, y):   # 获取波峰段
    ...
    # plt.plot(crest_segment_x, crest_segment_y, 'red', label="second_data")
    # plt.legend(loc=4)
    # plt.xlabel("number")
    # plt.ylabel("weight")
...
def gain_first_data(x, y):
    ...
    return gain_second_data(x, seg_y, maximum_value, minimum_value)
...
#     # plt.plot(x, seg_y, 'green', label="first_data")
#
#     if maximum_value == []:
#         maximum_value.append(np.mean(y))
#
...
#             maximum_value.append(y[max_value[i]])
#
#     # plt.plot(x, seg_y, 'red', label="second_data")
#     # plt.legend(loc=4)
#     # plt.xlabel("time")
#     # plt.ylabel("weight")
...
def fast_dataloader(type='train'):
    if type == 'train':
...
def fast_dataloader_test(type='train', path=None):
    if type == 'train':
        x, y, true_weight = gain_x_y_true_weight_data()
        label = ['max_crest', 'median_crest', 'mean_maximum_value', 'mean_crest', 'mode_crest', 'true_weight']
    elif type == 'predict':
    ...
    for i in range(len(y)):
        temp = []
        max_crest, median_crest, mean_maximum_value, mode_crest, mean_crest = gain_first_data(x[i], y[i])
        temp.append(max_crest)
        temp.append(median_crest)
        temp.append(mean_maximum_value)
        temp.append(mean_crest)
