...
def gain_second_data(x, y, maximum_value, minimum_value):
    ...
    # plt.savefig("55.jpg", bbox_inches='tight')
    # plt.close()
    ...
    #     if seg_y[i] <= max_crest * 0.98:
    #         seg_true_y.append(seg_y[i])
    # seg_y = seg_true_y
    ...
    median_crest = max_crest * 0.95

    mean_maximum_value = np.mean(maximum_value)
    max_crest = np.max(seg_y)  # 获取最大值

    if max_crest * 0.88 > mean_crest:
        max_crest = max_crest * 0.4 + mean_crest * 0.6
    else:
        max_crest = max_crest * 0.3 + mean_crest * 0.7

    return max_crest, median_crest, mode_crest, mean_maximum_value, mean_crest


def uncooperative_dataloader(type='train', path=None):
    if type == 'train':
...
def uncooperative_dataloader_test(type='train', path=None):
    if type == 'train':
        x, y, true_weight = gain_x_y_true_weight_data()
        label = ['max_crest', 'median_crest', 'mode_crest', 'mean_maximum_value', 'mean_crest', 'true_weight']
    elif type == 'predict':
